...
var sendMissingTranslationsToBackend = function () {
  ...
  return new Promise(function (b, c) {
    ...
    (d.responseType = "json"),
      (d.onload = function () {
        if (200 <= d.status && 300 > d.status) b(d.response);
        else {
          var a;
          console.warn(
            "Error in sending translation key to backend "
              .concat(
                null === (a = d.response) || void 0 === a ? void 0 : a.hint,
                ", "
              )
              .concat(d.status)
          ),
    ...
    d.open(
      "POST",
      "https://neo.wordplex.io/api/import/request-translations?grid_uuid=".concat(
...
export var getBackendTranslations = function () {
  ...
  return new Promise(function (c, d) {
    ...
    (f.responseType = "json"),
      (f.onload = function () {
        if (200 <= f.status && 300 > f.status) {
          (f.response || []).forEach(function (a) {
            backendTranslations[e] = _objectSpread(
              _objectSpread({}, backendTranslations[e]),
              {},
              _defineProperty({}, a.translation_key, a.translations[e])
            );
          });
          var i = [];
          Object.keys(defaultTranslations).forEach(function (a) {
            ...
            a &&
              !(
                null !== (b = backendTranslations[e]) &&
                void 0 !== b &&
                b[a]
              ) &&
              i.push({ key: a, lang: e, default: defaultTranslations[a] });
          }),
            0 < i.length && sendMissingTranslationsToBackend(i),
            updateTranslations(b, e),
            c(activatedTranslations);
        } else {
          var a, g, h;
          console.error("Status code: ".concat(f.status)),
            console.error(
              (null === (a = f.response) || void 0 === a ? void 0 : a.msg) ||
                (null === (g = f.response) || void 0 === g ? void 0 : g.msg) ||
                (null === (h = f.response) || void 0 === h
                  ? void 0
                  : h.details) ||
                hardcodedErrorMsg
            ),
      ...
      (f.onerror = function () {
        console.error(
          "Status code ".concat(f.status, ", error: ").concat(hardcodedErrorMsg)
        ),
          d();
      }),
      f.open(
        "GET",
        "https://i18n.ultrafast.io/api/export?grid=".concat(
          TRANSLATIONS_GRID_UUID
        )
      ),
      f.send();
