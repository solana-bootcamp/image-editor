var textarea,
  ...
  deactivateTextChange = function () {
    textarea && textarea.remove(textarea),
      window && window.removeEventListener("click", handleOutsideClick),
      textNode && textNode.show(),
      transformer && (transformer.show(), transformer.forceUpdate()),
      "function" == typeof disableTextEditCallback && disableTextEditCallback(),
      (textNode = null),
      (textarea = null),
      (transformer = null);
  },
  activateTextChange = function (a, b, c, d, e) {
    ...
    (editFinishCallback = d),
      (disableTextEditCallback = e),
      (transformer = c),
      (textNode = b.findOne("#".concat(a))),
      textNode.hide(),
      transformer.hide();
    var g = textNode.absolutePosition();
    (textarea = document.createElement("textarea")),
      b.container().parentNode.appendChild(textarea),
      (textarea.value = textNode.text()),
      (textarea.style.position = "absolute"),
      (textarea.style.top = "".concat(g.y, "px")),
      (textarea.style.left = "".concat(g.x, "px")),
      (textarea.style.width = "".concat(
        textNode.width() - 2 * textNode.padding(),
        "px"
      )),
      (textarea.style.height = "".concat(
        textNode.height() - 2 * textNode.padding() + 5,
        "px"
      )),
      (textarea.style.maxWidth = "".concat(
        textNode.width() - 2 * textNode.padding(),
        "px"
      )),
      (textarea.style.maxHeight = "".concat(
        textNode.height() - 2 * textNode.padding() + 5,
        "px"
      )),
    ...
    k && (j += 2 + Math.round(textNode.fontSize() / 20)),
      (i += "translateY(-".concat(j, "px)")),
      (textarea.style.transform = i),
