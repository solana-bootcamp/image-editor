import _defineProperty from "@babel/runtime/helpers/defineProperty";
var _SET_ORIGINAL_IMAGE$S;
import addFilter, { ADD_FILTER } from "./addFilter";
import hideLoader, { HIDE_LOADER } from "./hideLoader";
import redo, { REDO } from "./redo";
import reset, { RESET } from "./reset";
import setFeedback, { SET_FEEDBACK } from "./setFeedback";
import showLoader, { SHOW_LOADER } from "./showLoader";
import selectTab, { SELECT_TAB } from "./selectTab";
import undo, { UNDO } from "./undo";
import setFinetune, { SET_FINETUNE } from "./setFinetune";
import setAnnotation, { SET_ANNOTATION } from "./setAnnotation";
import changePointerIcon, { CHANGE_POINTER_ICON } from "./changePointerIcon";
import selectAnnotation, { SELECT_ANNOTATION } from "./selectAnnotation";
import clearAnnotationsSelections, {
  CLEAR_ANNOTATIONS_SELECTIONS,
} from "./clearAnnotationsSelections";
import toggleOriginalImageDisplay, {
  TOGGLE_ORIGINAL_IMAGE_DISPLAY,
} from "./toggleOriginalImageDisplay";
import selectTool, { SELECT_TOOL } from "./selectTool";
import toggleFlip, { TOGGLE_FLIP } from "./toggleFlip";
import setCrop, { SET_CROP } from "./setCrop";
import changeRotation, { CHANGE_ROTATION } from "./changeRotation";
import zoomCanvas, { ZOOM_CANVAS } from "./zoomCanvas";
import setCanvasSize, { SET_CANVAS_SIZE } from "./setCanvasSize";
import setOriginalImage, { SET_ORIGINAL_IMAGE } from "./setOriginalImage";
import setShownImageDimensions, {
  SET_SHOWN_IMAGE_DIMENSIONS,
} from "./setShownImageDimensions";
import removeAnnotations, { REMOVE_ANNOTATIONS } from "./removeAnnotations";
import duplicateAnnotations, {
  DUPLICATE_ANNOTATIONS,
} from "./duplicateAnnotations";
import enableTextContentEdit, {
  ENABLE_TEXT_CONTENT_EDIT,
} from "./enableTextContentEdit";
import setResize, { SET_RESIZE } from "./setResize";
import setSaved, { SET_SAVED } from "./setSaved";
import updateState, { UPDATE_STATE } from "./updateState";
import setLatestColor, { SET_LATEST_COLOR } from "./setLatestColor";
export default ((_SET_ORIGINAL_IMAGE$S = {}),
_defineProperty(_SET_ORIGINAL_IMAGE$S, SET_ORIGINAL_IMAGE, setOriginalImage),
_defineProperty(_SET_ORIGINAL_IMAGE$S, SET_FEEDBACK, setFeedback),
_defineProperty(_SET_ORIGINAL_IMAGE$S, SHOW_LOADER, showLoader),
_defineProperty(_SET_ORIGINAL_IMAGE$S, HIDE_LOADER, hideLoader),
_defineProperty(_SET_ORIGINAL_IMAGE$S, CHANGE_POINTER_ICON, changePointerIcon),
_defineProperty(_SET_ORIGINAL_IMAGE$S, SELECT_ANNOTATION, selectAnnotation),
_defineProperty(
  _SET_ORIGINAL_IMAGE$S,
  CLEAR_ANNOTATIONS_SELECTIONS,
  clearAnnotationsSelections
),
_defineProperty(
  _SET_ORIGINAL_IMAGE$S,
  TOGGLE_ORIGINAL_IMAGE_DISPLAY,
  toggleOriginalImageDisplay
),
_defineProperty(_SET_ORIGINAL_IMAGE$S, SELECT_TAB, selectTab),
_defineProperty(_SET_ORIGINAL_IMAGE$S, SELECT_TOOL, selectTool),
_defineProperty(_SET_ORIGINAL_IMAGE$S, ZOOM_CANVAS, zoomCanvas),
_defineProperty(_SET_ORIGINAL_IMAGE$S, SET_CANVAS_SIZE, setCanvasSize),
_defineProperty(
  _SET_ORIGINAL_IMAGE$S,
  SET_SHOWN_IMAGE_DIMENSIONS,
  setShownImageDimensions
),
_defineProperty(
  _SET_ORIGINAL_IMAGE$S,
  ENABLE_TEXT_CONTENT_EDIT,
  enableTextContentEdit
),
_defineProperty(_SET_ORIGINAL_IMAGE$S, SET_LATEST_COLOR, setLatestColor),
_defineProperty(_SET_ORIGINAL_IMAGE$S, ADD_FILTER, addFilter),
_defineProperty(_SET_ORIGINAL_IMAGE$S, SET_FINETUNE, setFinetune),
_defineProperty(_SET_ORIGINAL_IMAGE$S, SET_ANNOTATION, setAnnotation),
_defineProperty(_SET_ORIGINAL_IMAGE$S, SET_CROP, setCrop),
_defineProperty(_SET_ORIGINAL_IMAGE$S, CHANGE_ROTATION, changeRotation),
_defineProperty(_SET_ORIGINAL_IMAGE$S, TOGGLE_FLIP, toggleFlip),
_defineProperty(_SET_ORIGINAL_IMAGE$S, SET_RESIZE, setResize),
_defineProperty(_SET_ORIGINAL_IMAGE$S, REMOVE_ANNOTATIONS, removeAnnotations),
_defineProperty(
  _SET_ORIGINAL_IMAGE$S,
  DUPLICATE_ANNOTATIONS,
  duplicateAnnotations
),
_defineProperty(_SET_ORIGINAL_IMAGE$S, UNDO, undo),
_defineProperty(_SET_ORIGINAL_IMAGE$S, REDO, redo),
_defineProperty(_SET_ORIGINAL_IMAGE$S, RESET, reset),
_defineProperty(_SET_ORIGINAL_IMAGE$S, SET_SAVED, setSaved),
_defineProperty(_SET_ORIGINAL_IMAGE$S, UPDATE_STATE, updateState),
_SET_ORIGINAL_IMAGE$S);
export {
  SET_ORIGINAL_IMAGE,
  SET_FEEDBACK,
  SHOW_LOADER,
  HIDE_LOADER,
  CHANGE_POINTER_ICON,
  SELECT_ANNOTATION,
  CLEAR_ANNOTATIONS_SELECTIONS,
  TOGGLE_ORIGINAL_IMAGE_DISPLAY,
  SELECT_TAB,
  SELECT_TOOL,
  ZOOM_CANVAS,
  SET_CANVAS_SIZE,
  SET_SHOWN_IMAGE_DIMENSIONS,
  ENABLE_TEXT_CONTENT_EDIT,
  SET_LATEST_COLOR,
  ADD_FILTER,
  SET_FINETUNE,
  SET_ANNOTATION,
  SET_CROP,
  CHANGE_ROTATION,
  TOGGLE_FLIP,
  SET_RESIZE,
  REMOVE_ANNOTATIONS,
  DUPLICATE_ANNOTATIONS,
  UNDO,
  REDO,
  RESET,
  SET_SAVED,
  UPDATE_STATE,
};
